...
public class GatewayService extends Service {
    ...
    private static String dashboardHtml(SharedPreferences c) {
        ...
        h.append("<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'><title>9Router Mobile</title><style>")
         .append("body{font-family:Arial;background:#111;color:#eee;margin:0;padding:20px}.box{max-width:820px;margin:auto}.card{background:#1c1c1c;padding:16px;margin:12px 0;border-radius:12px}input{width:100%;box-sizing:border-box;padding:10px;margin:5px 0;background:#292929;color:#fff;border:1px solid #555;border-radius:7px}button{padding:12px 18px;border:0;border-radius:8px;font-weight:bold;margin:5px 4px 5px 0}.add{background:#2457d6;color:#fff}.save{background:#eee;color:#111}small{color:#aaa}.ok{color:#6f6}.bad{color:#f77}.provider-hidden{display:none}</style></head><body><div class='box'>")
         ...
         .append("<form method='post' action='/dashboard/save' id='cfgForm'><input type='hidden' id='providerCount' name='providerCount' value='").append(providerCount).append("'><div class='card'><h3>Gateway</h3><label>Public model alias</label><input name='model' value='").append(attr(c.getString("model","AGY"))).append("'><label>Local API key</label><input type='password' name='localKey' value='").append(attr(c.getString("localKey",""))).append("'></div>");

        for (int i=1; i<=MAX_PROVIDERS; i++) {
            boolean visible = i <= providerCount;
            h.append("<div class='card provider-card").append(visible ? "" : " provider-hidden").append("' id='providerCard").append(i).append("'><h3>Provider ").append(i).append(i==1?" · Primary":" · Fallback").append("</h3><label><input style='width:auto' type='checkbox' name='p").append(i).append("Enabled' ").append(c.getBoolean("p"+i+"Enabled",i==1)?"checked":"").append("> Enabled</label>")
        ...
        h.append("<button class='add' type='button' id='addFallback' onclick='addFallbackProvider()'>+ ADD FALLBACK</button><button class='save' type='submit'>SAVE CONFIG</button></form>")
         .append("<p><small>Maksimal ").append(MAX_PROVIDERS).append(" provider. Urutan provider menentukan prioritas fallback.</small></p>")
         .append("<p><small>API endpoint: /v1/chat/completions · /v1/models · /v1/providers</small></p>")
         .append("<script>function addFallbackProvider(){var c=document.getElementById('providerCount');var n=parseInt(c.value||'1',10);if(n>=").append(MAX_PROVIDERS).append("){alert('Maksimal ").append(MAX_PROVIDERS).append(" provider');return;}n++;c.value=n;var card=document.getElementById('providerCard'+n);if(card)card.className='card provider-card';card.scrollIntoView({behavior:'smooth',block:'center'});}</script>")
